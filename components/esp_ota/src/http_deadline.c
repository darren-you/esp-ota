...
static void expire_socket(void *argument)
{
    ...
    const int socket_fd = atomic_load_explicit(&deadline->socket_fd, memory_order_acquire);
    if (socket_fd >= 0) (void)shutdown(socket_fd, SHUT_RDWR);
}

bool eota_http_deadline_init(eota_http_deadline_t *deadline, int socket_fd)
{
    if (deadline == NULL || socket_fd < -1) return false;
    deadline->timer = NULL;
    atomic_init(&deadline->socket_fd, socket_fd);
...
{
    if (deadline == NULL || socket_fd < 0) return;
    atomic_store_explicit(&deadline->socket_fd, socket_fd, memory_order_release);
    if (atomic_load_explicit(&deadline->expired, memory_order_acquire)) {
