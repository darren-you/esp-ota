...
总期限从 DNS 前开始；无进展期限由 DNS 解析完成、TCP 连接完成及实际收发的网络字节刷新；DNS、TCP、TLS、请求发送、响应头和响应体共用这两项绝对期限，连接阶段另受 `connect_timeout_ms` 约束。单调时钟是唯一的期限事实，不创建到期定时器；DNS 每 tick 检查一次。单次 TLS 读写以 SDK 传入的 `read_timeout_ms` 建立绝对截止；该截止覆盖同一 TLS 记录的多次非阻塞收发，`select` 等待取它与总期限、无进展期限的最短剩余时间。进展必须先通过旧期限检查，逾期结果或字节不能续期；单次 TLS 密码学步骤若恰在读写截止后产出明文，传输先交还已消费的数据，以免丢失记录，并在下一步重新检查总期限。DNS 迟到回调由独立引用持有上下文，至多保留一个未完成解析。HTTP 返回后由同步调用方清理 client，再销毁 custom transport 并关闭 socket。
